...
class MultiHeadAttentionwDropout(nn.Module):
    ...
    def forward(self, 
        ...
        if self.flash_attn:
            ...
            out, attn = self.attn(query=query_w_pos, key=key_w_pos, value=value, attn_mask=attn_mask)
...
class OrionTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, attn_mask):
        """ Forward function for transformer decoder layer
        Args:
            query: shape [num_query, batch_size, embed_dims]
            key: shape [num_key, batch_size, embed_dims]
...
class OrionTransformerDecoder(nn.Module):
    ...
    def forward(self, query, key, query_pos=None, key_pos=None, attn_mask=None):
        """ Forward function for transformer decoder
        Args:
            query: shape [num_query, batch_size, embed_dims]
            key: shape [num_key, batch_size, embed_dims]
        ...
        for layer in self._layers:
            if self._with_cp and self.training:
                query = cp.checkpoint(layer, query, key, query_pos, key_pos, attn_mask)
            else:
                query = layer(query, key, query_pos, key_pos, attn_mask)
            return_val.append(query)
