...
class LoadAnnotations3D(LoadAnnotations):
    ...
    def _load_light_state(self, results):
        """Private function to load 3D mask annotations.
        ...
        Returns:
            dict: The dict containing loaded 3D mask annotations.
        """
        data_root = results['filename'][0].split('camera')[0]
        frame_idx = results['frame_idx']
        raw_anno_info_path = os.path.join(data_root,'anno',"{:05d}.json.gz".format(frame_idx))
        with gzip.open(os.path.join(raw_anno_info_path), 'rt', encoding='utf-8') as gz_file:
    ...
    def __call__(self, results):
        ...
        if self.with_light_state:
            results = self._load_light_state(results)
        return results
...
class LoadMultiViewImageFromFilesInCeph(object):
    ...
    def __call__(self, results):
        ...
        results['img'] = [img[..., i] for i in range(img.shape[-1])] # 900 x 1600 x 3
