...
class B2DCoTDataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        info_next = self.data_infos[index+1]
        if  info['folder'] != info_next['folder'] or info_next['frame_idx'] != info['frame_idx']+1 : #这个场景的最后一帧，没有video监督，不要
            return None
        else:
            pass #先预测一帧把，太多了我怕干不动
        ...
        for i in range(len(info['gt_names'])):
            ...
            if info_next['gt_names'][i] in self.NameMapping.keys():
                info_next['gt_names'][i] = self.NameMapping[info_next['gt_names'][i]]
